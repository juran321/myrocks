...
int Rdb_key_def::read_memcmp_key_part(const TABLE *table_arg,
  ...
  Field *field = nullptr;
  if (!is_hidden_pk_part) field = fpi->get_field_in_table(table_arg);
  if ((this->*fpi->m_skip_func)(fpi, field, reader)) return 1;
...
uint Rdb_key_def::pack_key_from_other_table(
    ...
    const Rdb_key_def &other_tbl_key_def, uchar *const pack_buffer,
    const uchar *const record, uchar *const packed_tuple) const {
  ...
  if (secondary_key) n_key_parts -= hidden_pk_exists ? 1 : m_pk_key_parts;

  for (uint i = 0; i < n_key_parts; i++) {
    Field *const other_tbl_field =
        other_tbl_key_def.get_field_packing()[i].get_field_in_table(other_tbl);
    ...
    uint other_tbl_field_null_offset =
        other_tbl_field->null_offset(other_tbl->record[0]);
    bool other_tbl_field_is_real_null =
        other_tbl_field->is_null_in_record(const_cast<uchar *>(record));

    Field *const field = m_pack_info[i].get_field_in_table(my_tbl);
    DBUG_ASSERT(field != nullptr);
    bool maybe_null = field->real_maybe_null();

    other_tbl_field->move_field(const_cast<uchar *>(record) +
                                other_tbl_field_offset);
    ...
    if (pack) {
      ...
      (this->*m_pack_info[i].m_pack_func)(&m_pack_info[i], other_tbl_field,
                                          pack_buffer, &tuple, &pack_ctx);

      // WARNING! Don't return without restoring field->ptr and field->null_ptr
    }
    // Restore field->ptr and field->null_ptr
    other_tbl_field->move_field(
        other_tbl->record[0] + other_tbl_field_offset,
        other_tbl_field_maybe_null
            ? other_tbl->record[0] + other_tbl_field_null_offset
            : nullptr,
...
size_t Rdb_key_def::key_length(const TABLE *const table,
  ...
  if ((!reader.read(INDEX_NUMBER_SIZE))) return size_t(-1);
  ...
  for (uint i = 0; i < m_key_parts; i++) {
    ...
    if ((this->*fpi->m_skip_func)(fpi, field, &reader)) return size_t(-1);
  }
  ...
  for (uint i = 0; i < m_key_parts; i++) {
    ...
    */
    ...
    if (fpi->m_unpack_func && covered_column) {
      ...
      if (fpi->m_maybe_null) {
        const char *nullp;
        if (!(nullp = reader.read(1))) return HA_ERR_ROCKSDB_CORRUPT_DATA;
        if (*nullp == 0) {
          /* This is a NULL value */
          continue;
        }
        /* If NULL marker is not '0', it can be only '1'  */
        if (*nullp != 1) return HA_ERR_ROCKSDB_CORRUPT_DATA;
      }
...
bool Rdb_ddl_manager::init(Rdb_dict_manager *const dict_arg,
  ...
  for (auto &fk_def : fk_def_vec) {
    auto foreign_tbl_it =
        m_index_num_to_keydef.find(fk_def.m_foreign_gl_index_id);
    ...
    auto referenced_tbl_it =
        m_index_num_to_keydef.find(fk_def.m_referenced_gl_index_id);
...
bool Rdb_ddl_manager::rename(const std::string &from, const std::string &to,
  ...
  if (to_is_tmp) {
    if (!new_rec->drop_fk_set.empty()) {
      ...
      for (auto it = rec->drop_fk_set.begin(); it != rec->drop_fk_set.end();
           ++it) {
        std::string id = *it;
        for (auto iter = fk_set.begin(); iter != fk_set.end(); ++iter) {
          ...
          if (cur_fk.id == id) {
            ...
            std::string referenced_table_name =
                ddl_manager->safe_get_table_name(
                    iter->m_referenced_gl_index_id);
...
*/
rocksdb::Slice Rdb_binlog_manager::pack_value(
    uchar *const buf, const char *const binlog_name, const my_off_t &binlog_pos,
...
void Rdb_dict_manager::put_fk_def(rocksdb::WriteBatch *const batch,
  ...
  rdb_netbuf_store_uint32(key_buf + 2 * Rdb_key_def::INDEX_NUMBER_SIZE,
                          foreign_gl_index_id.index_id);
  const rocksdb::Slice key = rocksdb::Slice((char *)key_buf, sizeof(key_buf));
